...
def test(dataset_name, model_dir, args):
    ...
    while i < len_dataloader:
        ...
        for idx, prediction in enumerate(class_output):
          #pred_tsne = prediction.data.cpu().numpy()
          prediction = torch.argmax(prediction.squeeze(), dim=0).detach().cpu().numpy()
          file_name = extract_filename(t_path[idx])
          #logits_list.append(pred_tsne)     
          preds.append((file_name, int(prediction)))


        i += 1
    #logits_list = np.concatenate(logits_list)
    #logits_list = np.reshape(logits_list,(10000,10))
    #tsne = manifold.TSNE(n_components=2, verbose=1, perplexity=10, n_iter=1000) 
    #tsne_results = tsne.fit_transform(logits_list)    

    #target_ids = range(len(preds))
    #print(target_ids)

    #plt.scatter(tsne_results[:,1],tsne_results[:,0],c = target_ids ,s=2,cmap=plt.cm.get_cmap("jet", 10))
