...
if __name__=='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        while idx < dataset_length:
            ...
            print("[Err Source Label: %f]"%(err_s_label.data.cpu().numpy()))
            print("[Err Source Domain Label: %f]"%(err_s_domain.data.cpu().numpy()))
            print("[Err Target Domain Label: %f]"%(err_t_domain.data.cpu().item()))
        # save the model
        torch.save(dann, '{0}_for_lower_higher.pth'.format(target))
